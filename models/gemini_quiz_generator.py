...
class GeminiQuizGenerator:
    """
    Generates multiple-choice quiz questions from text using Google's Gemini API.
    A faster (1-2 seconds) and more accurate alternative to running T5 locally on CPU.
    """

    _instance = None  # Singleton pattern

    @classmethod
    def get_instance(cls) -> "GeminiQuizGenerator":
    ...
    def __init__(self):
        # Import the library dynamically to avoid crashing if the package is not installed yet
    ...
    def _init_client(self):
        ...
        if not api_key:
            print("[Gemini-QuizGen] Warning: GEMINI_API_KEY environment variable is not set in your .env file.")
        ...
        try:
            ...
            print("[Gemini-QuizGen] Error: 'google-generativeai' package is not installed.")
            print("[Gemini-QuizGen] To use this generator, please run: pip install google-generativeai")
            raise
    ...
    def generate_quiz(
        ...
        if not flashcard_pairs:
            print("[Gemini-QuizGen] Error: No flashcards available for quiz generation.")
            return []
        ...
        if is_descriptive:
            print(f"[Gemini-QuizGen] Descriptive deck detected. Generating contextual multiple-choice quiz...")
            ...
            if quiz_data:
                print(f"[Gemini-QuizGen] Descriptive quiz generated successfully with {len(quiz_data)} questions.")
    ...
    def _generate_descriptive_quiz_questions(self, text: str, flashcards: List[Dict], needed: int = 20, content_level: str = "Medium") -> List[Dict]:
        """
        Generates high-quality multiple-choice questions directly from descriptive flashcards using Gemini.
...
5. Output MUST be a strictly valid JSON array of objects without markdown wrappers:
[
  {{
...
"""
            ...
            if len(normalized) < needed:
                ...
                print(f"[Gemini-QuizGen] Descriptive quiz generated {len(normalized)} items. Requesting {rem} more...")
